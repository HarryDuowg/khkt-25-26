...
void normal() {
  ...
  switch(state) {
    case 0: // XANH L1-2
      ...
      if (time1 == 0) { 
        state = 1; 
        time1 = (l12 < N_XE && l34 < N_XE) ? 2 : 3; // vang
        special = false; 
      }
      break;
    case 1: // VÀNG L1-2
...
void loop() {
  if (Serial.available()) {
    ...
    if (s.startsWith("L1")) {
      ...
      // Nếu L1-2 xanh, có xe mà L3-4 trống -> Cộng 7s cho L1-2
      if (state == 0 && !special && (L[0] + L[1] > 0) && (L[2] + L[3] == 0)) {
         time1 += 4; time2 += 4;
         special = true; 
      }
      
      // Nếu L3-4 xanh, có xe mà L1-2 trống -> Cộng 7s cho L3-4
